...
class DetectBoxes:
    ...
    def bounding_box_yolo(self, frame, input_dim, model):
        img, orig_im, dim = prep_image(frame, input_dim)
        image_dim = torch.FloatTensor(dim).repeat(1, 2).to(self.device)
        ...
        if detections is not None:
            image_dim = image_dim.repeat(detections.size(0), 1)
            scaling_factor = torch.min(input_dim / image_dim, 1)[0].view(-1, 1)

            detections[:, [0, 2]] -= ((input_dim - scaling_factor * image_dim[:, 0].view(-1, 1)) / 2).cpu()
            detections[:, [1, 3]] -= ((input_dim - scaling_factor * image_dim[:, 1].view(-1, 1)) / 2).cpu()
